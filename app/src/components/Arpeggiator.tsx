...
export const Arpeggiator: React.FC = () => {
  ...
  const handleOctave = (e: React.ChangeEvent<HTMLInputElement>) => {
    setOctave(e.target.value);
    const val = e.target.value === '3' ? 100 : 64; 
    midiService.sendCC(channel, 8, val);
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 flex-1">
        ...
        <div className="space-y-6 flex flex-col justify-center">
          <div className="flex justify-between items-end border-b border-outline-variant/30 pb-2 relative group">
            ...
            <span className="font-headline text-secondary text-xs font-bold pointer-events-none">SYNC_I</span>
            <select aria-label="Pattern" className="absolute inset-0 opacity-0 cursor-pointer z-10 w-full h-full" onChange={(e) => playPattern(Number(e.target.value))}>
          ...
          <div className="flex justify-between items-end border-b border-outline-variant/30 pb-2 relative group">
            ...
            <span className="font-headline text-secondary text-xs font-bold pointer-events-none">{octave}_OCT</span>
            <input type="number" aria-label="Octave" value={octave} onChange={handleOctave} className="absolute inset-0 opacity-0 cursor-pointer z-10 w-full h-full" />
          </div>
          <div className="flex justify-between items-end border-b border-outline-variant/30 pb-2 relative group">
