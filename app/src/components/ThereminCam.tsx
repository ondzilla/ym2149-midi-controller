...
export const ThereminCam: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const processFrame = () => {
      ...
      // ⚡ Bolt Optimization: Cache the CanvasRenderingContext2D to avoid
      // calling getContext('2d') every frame in the requestAnimationFrame loop
      // which incurs unnecessary garbage collection pressure and CPU overhead.
      if (!ctxRef.current) {
      ...
      // ⚡ Bolt Optimization: Track 1D array index `i` sequentially to avoid
      // (y * RESOLUTION_WIDTH + x) * 4 recalculation for 3072 pixels per frame,
      // saving ~552,960 math operations per second at 60fps.
      let i = 0;
