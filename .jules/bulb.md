...
## 2026-05-16 - Web Bluetooth API for Continuous MIDI
Learning: The Web Bluetooth API (`navigator.bluetooth`) is a verified zero-dependency Web API suitable for experimental desktop features. It allows connecting to BLE peripherals (like heart rate monitors) to generate continuous MIDI CC control streams without heavy external libraries.
Action: Documented the "BLE Heart Rate Modulator" experimental feature, exploring how to stream physiological BPM data into standard MIDI CC messages for the YM2149 synthesizer.
