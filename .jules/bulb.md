...
## 2026-05-19 - Web Bluetooth API for Organic MIDI Generation
Learning: The Web Bluetooth API (`navigator.bluetooth`) provides a lightweight, zero-dependency mechanism to connect to BLE devices (like heart rate monitors). This is exceptionally well-suited for generating an organic, continuous MIDI stream by mapping biological signals (like BPM) to time-based synth parameters (e.g., Arpeggiation or Vibrato Rate).
Action: Proposed the "Biometric Pulse Modulator" experimental feature, leveraging this API to transform the performer's body into a dynamic MIDI controller while maintaining strict hardware parity with YM2149F CCs.
