...
## 2025-05-08 - Icon Ligature Accessibility
**Learning:** When using ligature-based icon fonts (like Material Symbols) inside interactive elements, applying `aria-hidden="true"` to the parent `<button>` or `<a>` improperly removes the entire element from the accessibility tree, hiding it from screen readers. Conversely, failing to hide the text nodes containing raw ligature strings (e.g., "power_settings_new") causes screen readers to read unintelligible button labels.
**Action:** When using ligature-based icon fonts inside interactive elements, strictly apply `aria-hidden="true"` to the icon element itself (e.g., `<span className="material-symbols-outlined" aria-hidden="true">`) and ensure the parent element has an appropriate `aria-label`.
